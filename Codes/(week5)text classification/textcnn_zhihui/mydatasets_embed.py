...
class My_datasets:
    ...
    def read_train_test_data(self):
        ...
        labels_pos = []
        count = 0
        with open(self.read_path + "/labeledData/"+self.workflow+"_cc_"+self.keywords+"_0.json", "r") as neg_file:
            for line in neg_file:
                if line.startswith("{"):
                    count += 1
                    json_data = json.loads(line)
                    tweets_neg.append(json_data["text"])
                    labels_neg.append(0)
        count = 0
        with open(self.read_path + "/labeledData/"+self.workflow+"_cc_"+self.keywords+"_1.json", "r") as pos_file:
            for line in pos_file:
                if line.startswith("{"):
                    count += 1
                    json_data = json.loads(line)
                    tweets_pos.append(json_data["text"])
                    labels_pos.append(1)
        random.seed(1)
    ...
    def data_processing(self, tweet_texts_Train, tweet_texts_Test, tweet_texts_Validate):
        # glove2word2vec(glove_input_file=self.read_path + "/word2vec/glove.twitter.27B.100d.txt", word2vec_output_file=self.read_path+"/word2vec/gensim_glove_vectors.txt")
        glove_model = KeyedVectors.load_word2vec_format(self.read_path+"/word2vec/gensim_glove_vectors.txt", binary=False)
        vocab = glove_model.vocab
        # vectorizer = CountVectorizer()
        # vectorizer.fit(tweet_texts_Train)
    ...
    def get_train_validate_test_loader(self):
        tweet_texts_Train, tweet_labels_Train, tweet_texts_Test, tweet_labels_Test, tweet_texts_Validate, tweet_labels_Validate = self.read_train_test_data()
        vocab_len, weights, feature_padding_train, feature_padding_test, feature_padding_validate = self.data_processing(tweet_texts_Train,
                                                                                                tweet_texts_Test,
                                                                                                tweet_texts_Validate)
